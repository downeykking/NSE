...
class Trainer(AbstractTrainer):
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        return total_loss, total_check_loss
    ...
    def _generate_train_loss_output(self, epoch_idx, s_time, e_time, total_train_time, losses, check_loss=None):
        ...
        if isinstance(losses, tuple):
            ...
            train_loss_output += ", ".join(
                des % (idx + 1, loss/self.config['train_batch_size']) for idx, loss in enumerate(losses)
            )
        else:
            des = "%." + str(des) + "f"
            train_loss_output += set_color("train loss", "blue") + ": " + des % (losses/self.config['train_batch_size'])
        if check_loss is not None:
            des = set_color("au_loss%d", "blue") + ": " + str(des)
            train_loss_output += ", ".join(
                des % (idx + 1, loss/self.config['train_batch_size']) for idx, loss in enumerate(check_loss)
            )
    ...
    def fit(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            train_loss, check_loss = self._train_epoch(
            ...
            if check_loss is not None:
                self.check_loss_dict[epoch_idx] = check_loss
            training_end_time = time()
            total_train_time += (training_end_time-training_start_time)
            train_loss_output = self._generate_train_loss_output(
                epoch_idx, training_start_time, training_end_time, total_train_time, train_loss, check_loss
            )
            if verbose:
                self.logger.info(train_loss_output)
            self._add_train_loss_to_tensorboard(epoch_idx, train_loss)
            if check_loss is not None:
                self._add_train_loss_to_tensorboard(epoch_idx, check_loss, tag="AULoss/Train")
    ...
    def evaluate(
        ...
        if load_best_model:
            ...
            self.model.load_state_dict(checkpoint["state_dict"])
            self.model.load_other_parameter(checkpoint.get("other_parameter"))
            message_output = "Loading model structure and parameters at epoch {} from {}".format(
                best_epoch, checkpoint_file
            )
            self.logger.info(message_output)
