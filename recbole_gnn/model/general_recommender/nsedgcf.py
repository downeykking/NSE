...
class NSEDGCF(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        super(NSEDGCF, self).__init__(config, dataset)
        ...
        self.edge2head_mat = SparseTensor.from_torch_sparse_coo_tensor(self.edge2head_mat)

        self.head2edge_mat = self._build_sparse_tensor(
            self.head2edge, val_one, (num_edge, num_node)
        )
        self.head2edge_mat = SparseTensor.from_torch_sparse_coo_tensor(self.head2edge_mat)

        self.tail2edge_mat = self._build_sparse_tensor(
            self.tail2edge, val_one, (num_edge, num_node)
        )
        self.tail2edge_mat = SparseTensor.from_torch_sparse_coo_tensor(self.tail2edge_mat)
    ...
    def build_matrix(self, A_values):
        ...
        for i in range(self.n_factors):
            ...
            # (num_edge, 1)
            d_values = spmm(self.edge2head_mat, tp_values)
            ...
            head_term = spmm(self.head2edge_mat, d_values)
            # (num_edge, num_node) (num_node, 1) -> (num_edge, 1)

            tail_term = spmm(self.tail2edge_mat, d_values)
        ...
        for k in range(self.n_layers):
            ...
            for t in range(0, self.n_iterations):
                ...
                for i in range(0, self.n_factors):
                    ...
                    # (num_edge, dim / n_factors)
                    factor_embeddings = spmm(self.edge2head_mat, edge_val)
